...
def into_aroma_dict(start, finish, weight):#ListArcs = [], ):
    #lists = {}
    global dict_of_aroma_bonds
    #input_lines_number = int(input()) if ListArcs == [] \
                         #else ListArcs[0]
    #for i in range(1,input_lines_number):
    ''' = bond input().split() if ListArcs == [] \
                                else ListArcs[i].split() #A B 5'''
    #weight = int(weight)
    ...
    for atom in dict_of_aroma_bonds:
        if atom not in three_valencies:
            ...
            if type_bond_l == 2:
                ...
                #dict_of_aroma_bonds[atom][last_atom].append(type_bond_l)
